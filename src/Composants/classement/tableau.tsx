...
import {DataToken, listUser} from "../../Interface/Interface.ts";
...
function Tableau(): JSX.Element {
    ...
    const [users, setUsers] = useState<listUser>({item: [], total: 0});
    const [submitCount, setSubmitCount] = useState(0);
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [itemPerPage, setItemPerPage] = useState(10);

    async function getUsers() {
        return await getElementByEndpoint(`user/getUsers?page=${currentPage}&itemPerPage=${itemPerPage}`, {
            token,
            data: '',
        });
    }
    ...
    async function onSearch(username: string) {
        const result = await getElementByEndpoint(`user/getUsersByUsername?username=${username}&itemPerPage=${itemPerPage}`, {
            token,
            data: '',
        });
    ...
    return (
        <div className="m-5">
            <div className="flex max-md:flex-col flex-row max-md:text-center justify-between">
                ...
                <SearchBar onSearch={onSearch} setItemPerPage={setItemPerPage} placeholder={"serachByUsername"} setCurrentPage={setCurrentPage}/>
            </div>
            <div className="flex flex-col justify-center w-full">
                ...
                <Pagination item={users.item} maxPage={maxPage} currentPage={currentPage} setCurrentPage={setCurrentPage}
                            setSubmitCount={setSubmitCount} classNameCurrentPage="text-tertiari" itemPerPage={itemPerPage}/>
            </div>
        </div>
    )
}

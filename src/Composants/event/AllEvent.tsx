...
function AllEvent({ infosEvents, className }: TableauEventProps) {
  ...
  // Function to handle redirection to event details
          ...
          >
            {infosEvents.map((item: Event, index: number) => (
              <FadeIn key={index} className="flex flex-col items-center">
                <Card
                  className="bg-tertiari rounded-xl m-3 sm:m-4 lg:m-5 border-tertiary shadow-md w-full cursor-pointer 
                                    hover:shadow-xl hover:scale-105 transition-transform duration-300"
                >
                  <CardContent className="text-secondary p-4 sm:p-5">
                    <div className="flex flex-col items-center" onClick={() => item.id && handleEventClick(item.id)}>
